...
//The asynchronous calculation needs to:
...
//The output of 2. and 3. each comes out through a separate channel (avgPrices and fiveDayAverages, respectively).
//These two output channels are consumed by tasks that print the values on the screen.

//TASK Implement the missing asynchronous operations, possibly connected with additional channels, so that the calculation passes.
//The first operation will convert Chicago prices from USD to EUR
//The second operation will calculate a daily average price in EUR using the three daily prices from different locations as its input
//The third operation will take the daily average prices calculated by the second operator and output a five-day moving average (an average of the last five days, including today).
//If the history is shorter than five days, use only the days that are available to calculate the average.
//All the operations need to run mutually asynchronously.
//
//Hint for GPars operator syntax:
/*
...
//Dummy price and exchange rate data for 30 days
...
group.with {
    ...
    assert results['fiveDayAveragesKey'] == [9, 10, 10, 10, 10, 9, 9, 8, 7, 7, 7, 7, 8, 9, 10, 11, 12, 12, 12, 12, 11, 11, 10, 10, 10, 11, 11, 12, 13, 12]
}
